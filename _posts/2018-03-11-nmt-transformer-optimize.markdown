...
The results below shows that TVM generated kernel(with schdule optimization) brings at least <b>*13X*</b> speed-up for batch matmul computation. The kernel fusion optimization brings further <b>*1.7X*</b> speed-up.(The shape of [batch=64, heads=8, M=1, N=17, K=128] is chosen to elaborate the performance of the generated code. 17 is chosen as the sequenc length since it is the average input length in our production scenarios.)
...
With the help of TVM, an end-to-end speed-up of <b>*1.4X*</b> has been achieved for our transformer based NMT system.
...
The generated efficient kernels for the specific shapes and the fall-back one are integrated into Tensorflow framework. Also a graph optimization pass is developed to automatically replace the origin batch *matmul + add/transpose* pattern with the TVM kernel.
